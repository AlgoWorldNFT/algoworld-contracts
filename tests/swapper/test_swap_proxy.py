...
def test_swap_proxy_activate_or_save(swap_creator, swap_proxy):
    ...
    with pytest.raises(AlgodHTTPError):
        activate_or_save_proxy_note(
            swap_creator, swap_proxy, "gotta_save_this", 10_000, 2_000, 0
        )

    # first txn is activation (should also fail if fee is less than required)
    with pytest.raises(AlgodHTTPError):
        activate_or_save_proxy_note(
            swap_creator, swap_proxy, "gotta_save_this", 110_000, 0, 0
        )

    # first txn is activation
    activate_or_save_proxy_note(
        swap_creator, swap_proxy, "ipfs://_gotta_save_this", 110_000, 2_000, 0
    )

    # second txn is a store operation
    activate_or_save_proxy_note(
        swap_creator, swap_proxy, "ipfs://_gotta_save_this", 10_000, 2_000, 0
    )
    ...
    with pytest.raises(AlgodHTTPError):
        activate_or_save_proxy_note(
            swap_proxy, swap_creator, "ipfs://_gotta_save_this", 10_000, 2_000, 0
        )

    with pytest.raises(AlgodHTTPError):
        activate_or_save_proxy_note(
            swap_creator, swap_proxy, "ipfs://_gotta_save_this", 10_000, 2_000, 1
        )

    with pytest.raises(AlgodHTTPError):
        activate_or_save_proxy_note(
            swap_creator, swap_proxy, "ipfs://_gotta_save_this", 0, 2_000, 10
        )
